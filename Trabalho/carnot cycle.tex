...
Tirando as constantes da integração, resta apenas integrar 1/V:
...
Onde $W = Q$, já que não há diferença na temperatura. Para o processo de compressão isotérmica 
...
do outro. Essa relação será importante na hora de calcular a eficiência, já que não precisaremos representá-los.
...
Do termo de capacidade térmica molar à pressão constante $C_p$ podemos descrever que:
...
se ${T_b}/{T_a} = 0$. Ou seja, se $T_b = 0$ ou $T_a = \infty$. Ou seja, se a temperatura do ferro mais frio 
for zero absoluto ou a temperatura do ferro mais quente for infinita. 
...
que vir de algum lugar e não energia infinita no universo.
...
Um motor 100\% precisaria de um sistema que não interage com o mundo fora dele em nenhuma forma.
...
segunda leis. Embora o motor de Carnot permaneça uma construção teórica inestimável para entender os limites 
